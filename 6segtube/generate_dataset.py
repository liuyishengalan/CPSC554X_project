...
class CustomDataset(Dataset):
    def __init__(self, data_file, labels_file, transform=None):
        ...
        for i in range(len(self.mfcc)):
            self.data.append(np.hstack((self.formants[i].reshape(-1),self.mfcc[i].reshape(-1) , self.td[i].reshape(-1)))) # self.mfcc[i].reshape(-1),
        '''
...
class CustomLoss(nn.Module):
    ...
    def forward(self, predicted, target):
        ...
        loss_5 = loss_function(predicted[:,4], target[:,4])  # Loss for second output
        loss_6 = loss_function(predicted[:,5], target[:,5])  # Loss for second output
        loss_7 = loss_function(predicted[:,6], target[:,6])  # Loss for second output


        total_loss = loss_1 + loss_2 + loss_3 + loss_4 + loss_5 + loss_6 + loss_7# Combine the losses
